 ...
 * @{
 * @addtogroup Naze32 Naze32 support files
 * @{
 *
 * @file       pios_board.c
...
static struct pios_mpu60x0_cfg pios_mpu6050_cfg = {
...
static const struct pios_exti_cfg pios_exti_hmc5883_cfg_v5 __exti_config = {
 ...
 * 3 pulses: I2C     - Internal I2C bus locked
 * 4 pulses: ms5611
 * 5 pulses: flash
 * 6 pulses: hmc5883l
 */
...
void PIOS_Board_Init(void) {
	...
	uint8_t hw_DSMxBind;
	HwNaze32DSMxBindGet(&hw_DSMxBind);
	...
	if (hw_rcvrport == HWNAZE32_RCVRPORT_PPM) {
		/* UART2 Port */
		uint8_t hw_uart2;
		
		HwNaze32Uart2Get(&hw_uart2);
		
		switch (hw_uart2)
		{
		case HWNAZE32_UART2_DISABLED:	
			break;
			
		case HWNAZE32_UART2_GPS:
		...
		case HWNAZE32_UART2_MAVLINKTX_GPS_RX:
	...
	if (board_v5) 
	{ 
		// rev5 hardware use PC14 instead of PB12 for HMC5883_INT
